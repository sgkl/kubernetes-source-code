...
#### Framework
Framework的接口，所有framework都要实现里面定义的函数
```go
...
#### PluginsRunner
将运行部分plugins的函数提出来作为单独的接口。因为这些接口在PostFilter plugins中被调用来评估驱逐的可行性。
```go
// PluginsRunner abstracts operations to run some plugins.
...
QueueSort plugin的接口，一个Framework只能有一个QueueSort
...
PreFilter plugin需要实现的接口。PreFilterExtensions是可以在PreFilter plugin中添加的扩展
...
需要注意的是，如果过滤函数可以返回四种状态：

* Success 成功
* Unschedulable 调度失败，但是调度器可以尝试通过驱逐其他pod来让这个pod可以被调度
* UnschedulableAndUnresolvable  调度失败，并且跳过驱逐过程。（因为调度失败的原因不是资源不足）
* Error plugin内部错误
...
pod不能被调度时，调用这个plugin进行驱逐，选出提名node。

PostFilter函数返回以下三个状态：

* Unschedulable：plugin成功运行，但是不能通过驱逐其他pod让此pod可调度
* Success：plugin成功运行，可以通过驱逐其他pod让此pod可调度
* Error：plugin运行失败，plugin遇到内部错误
...
在Score plugin前运行的plugin，可以用来更新调度过程的内部状态或进行一些薄记工作
...
为选出的可用node打分。其中ScoreExtensions是可以添加的扩展
...
在pod被binding到node前，将pod需要的资源在node中Reserve或unreserve，防止node中的资源被其他pod使用。
...
在PreBindPlugin前被调用，阻止或延迟pod的bind。
...
将pod和选出的运行此pod的node的绑定关系通过api server写入etcd
